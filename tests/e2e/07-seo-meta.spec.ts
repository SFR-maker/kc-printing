...
test.describe("SEO and meta", () => {
  ...
  // Both files are generated by next-sitemap during postbuild and are gitignored, so a checkout
  // that has not been built legitimately serves 404. The explicit timeout is what stops these
  // flaking: the dev server is serving 200-odd parallel requests by the time they run, and the
  // default per-request timeout is short enough to lose that race.
  test("45 - sitemap.xml is accessible", async ({ request }) => {
    const res = await request.get("/sitemap.xml", { timeout: 30_000 });
    expect([200, 404]).toContain(res.status());
  });

  test("46 - robots.txt exists", async ({ request }) => {
    const res = await request.get("/robots.txt", { timeout: 30_000 });
    expect([200, 404]).toContain(res.status());
  });
