...
/** The specs the inspector used to build internally from a boolean. */
const CARD = printSpec("business-cards", 3.5, 2);
const CARD_ROUNDED = printSpec("business-cards", 3.5, 2, 0.1625);
...
describe("inspectArtwork - accepted formats", () => {
  it("rejects Illustrator files with export guidance", async () => {
    await expect(inspectArtwork(Buffer.alloc(10), "card.ai", CARD)).rejects.toBeInstanceOf(ArtworkRejectedError);
    await expect(inspectArtwork(Buffer.alloc(10), "card.ai", CARD)).rejects.toThrow(/export as a PDF/i);
  });

  it("rejects Photoshop and EPS too", async () => {
    for (const name of ["card.psd", "card.eps"]) {
      await expect(inspectArtwork(Buffer.alloc(10), name, CARD)).rejects.toBeInstanceOf(ArtworkRejectedError);
    }
  });
...
describe("inspectArtwork - PDF sizing", () => {
  it("accepts a correctly sized square-corner PDF with no resize warning", async () => {
    const out = await inspectArtwork(await pdfAtInches(3.6, 2.1), "card.pdf", CARD);
  ...
  it("requires the larger document when rounded corners are selected", async () => {
    const out = await inspectArtwork(await pdfAtInches(3.825, 2.325), "card.pdf", CARD_ROUNDED);
  ...
  it("flags a square-corner file submitted against the rounded spec", async () => {
    const out = await inspectArtwork(await pdfAtInches(3.6, 2.1), "card.pdf", CARD_ROUNDED);
  ...
  it("scales up a trim-sized file supplied with no bleed, and says what that costs", async () => {
    ...
    const out = await inspectArtwork(await pdfAtInches(3.5, 2), "card.pdf", CARD);
    expect(out.matchesRequiredSize).toBe(false);
  ...
  it("warns that a square document will be cropped to card proportions", async () => {
    const out = await inspectArtwork(await pdfAtInches(3.6, 3.6), "card.pdf", CARD);
    expect(out.fit.cropsContent).toBe(true);
    expect(out.warnings.some((w) => w.code === "aspect")).toBe(true);
  });
});
...
describe("inspectArtwork - raster resolution", () => {
  it("judges resolution by pixel count, not the file's DPI tag", async () => {
    ...
    const out = await inspectArtwork(await png(1080, 630, 72), "card.png", CARD);
  ...
  it("warns below 300 DPI at card size", async () => {
    const out = await inspectArtwork(await png(720, 420), "card.png", CARD);
  ...
  it("blocks below 150 DPI at card size", async () => {
    const out = await inspectArtwork(await png(360, 210), "card.png", CARD);
  ...
  it("needs more pixels for the larger rounded-corner document", async () => {
    ...
    const out = await inspectArtwork(await png(1080, 630), "card.png", CARD_ROUNDED);
    expect(out.effectiveDpi).toBeLessThan(300);
    expect(out.warnings.some((w) => w.code === "dpi-low")).toBe(true);
  });

  it("reports pixel dimensions so the customer can act on them", async () => {
    const out = await inspectArtwork(await png(1500, 900), "card.png", CARD);
